...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy(MyBigInteger  divisor ) throws Exception{
		...
		if (divisor.coefficients.get(divisor.coefficients.size()-1) == 0) {
			throw new RuntimeException("Cannot divide by zero");
		}
		
		if (this.base != divisor.base){
			throw new RuntimeException("Bases must be the same when dividing");
		}
		...
		int coefLeft = this.coefficients.size()  ;
		int q = 0;
		MyBigInteger remainder = new MyBigInteger(this.base);
		int j=0;
		coefLeft = this.coefficients.size()-1;
		
		while(coefLeft >=0) {
			remainder.coefficients.add(0,this.coefficients.get(this.coefficients.size()-1-j));
			j++;
		
			
		while((remainder.coefficients.size() >  1) && (remainder.coefficients.get(remainder.coefficients.size()-1) ==  0)) {
			remainder.coefficients.remove(remainder.coefficients.size()-1);
		} 
	
		MyBigInteger temp  = remainder.clone();
		...
		quotient.coefficients.add(0, q);
		q = 0;
		remainder =  temp.clone();
		...
		while (quotient.coefficients.get(quotient.coefficients.size()-1) == 0) {
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		MyBigInteger Zero = new MyBigInteger(0,this.base);
		MyBigInteger tempDivide = new  MyBigInteger(this.base);
		tempDivide = this.clone();
		MyBigInteger  newBigBase =  new MyBigInteger(newBase,this.base);
		int remainder  = 0;
		
		if (newBase > 10 || newBase <2) {
			throw new Exception("You must convert into  a base between 2 and 10");
		}
		...
		MyBigInteger convertedNewBase =  new MyBigInteger(newBase,10);
		newBigBase = convertedNewBase.convert(this.base);	
		
		}

		while (tempDivide.compareTo(Zero) == 1) {
			MyBigInteger tempMod = tempDivide.clone();
			tempDivide = tempDivide.dividedBy(newBigBase);
			tempMod = tempMod.mod(newBigBase);
			for (int i = 0; i < tempMod.coefficients.size(); i++) {
				remainder += tempMod.coefficients.get(i) * (int) Math.pow(this.base, i);
				}
			result.coefficients.add(remainder);
			remainder = 0;
	...
	//Wheel factorization returns arrayList of prime factors of this
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		if (number.compareTo(two) < 0) {
			throw new IllegalArgumentException("No prime numbers.");
		}
		...
		while (number.mod(two).compareTo(zero) == 0 ) {
			...
			MyBigInteger jay = new MyBigInteger(3,this.base);
			while (number.compareTo(jay.times(jay)) >= 0) {
				
				while (number.mod(jay).compareTo(zero) == 0) {
					
					factors.add(jay);
					number =number.dividedBy(jay);
				}
	...
	*/
	...
	public MyBigInteger mod( MyBigInteger  divisor ) throws Exception{
		...
		Mod = this.minus(this.dividedBy(divisor).times(divisor));		
